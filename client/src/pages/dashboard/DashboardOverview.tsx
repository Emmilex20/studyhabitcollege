...
// 🎯 IMPORT ALL INTERFACES FROM THE CENTRALIZED TYPES FILE
// 🎯 IMPORT ALL INTERFACES FROM THE CENTRALIZED TYPES FILE
import type { AdminDashboardData, TeacherDashboardData, StudentDashboardData, Course } from '../../types';
...
const DashboardOverview: React.FC = () => {
  ...
  // Ensure selectedCourse uses the imported Course type
  ...
  const handleAddNewUser = async (userData: { firstName: string; lastName: string; email: string; password?: string; role: string }) => {
    ...
    try {
      ...
      setIsAddUserModal(false); // Close modal on success
  ...
  const fetchStudentData = useCallback(async () => {
    try {
      ...
      setStudentData({
        ...
        letterGrade: gpaRes.data.letterGrade || 'N/A',
    ...
    if (userInfo && userToken) {
      ...
      switch (userInfo.role) {
        ...
        case 'student':
          ...
          break;
        case 'parent':
          // ParentDashboardOverview component handles its own data fetching/loading
          setLoading(false); // Set loading to false as ParentDashboardOverview takes over
          break;
        default:
          setLoading(false);
